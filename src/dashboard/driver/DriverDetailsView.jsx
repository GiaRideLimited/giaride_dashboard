...
const DriverDetailsView = ({ reference: propReference, onBack }) => {
    ...
    useEffect(() => {
        if (activeTab === 'Ride History' && reference) {
            ...
            const endpoint = `${BASE_URL}/admin/driver-ride-history/${reference}`;
            fetch(endpoint)
                .then((response) => {
    ...
    useEffect(() => {
        if (activeTab === 'Earnings History' && reference) {
            ...
            const endpoint = `${BASE_URL}/admin/driver-earning-history/${reference}`;
            fetch(endpoint)
                .then((response) => {
                    if (!response.ok) throw new Error(`Server responded with ${response.status}`);
                    return response.json();
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <button
                        ...
                        className={`flex-1 md:flex-none px-6 py-2.5 border rounded-full text-sm font-medium transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed
                            ${driver.status === 'active'
                                ? 'text-red-600 border-red-100 hover:bg-red-50'
                                : 'text-green-600 border-green-100 hover:bg-green-50'}
                        `}
                    >
                        {isActionLoading ? 'Processing...' : driver.status === 'active' ? 'Deactivate Driver' : 'Activate Driver'}
                    </button>
                    <button
                                        ...
                                        {isHistoryLoading ? (
                                            ...
                                            rideHistory.map((ride, idx) => (
                                                <tr key={idx} className="group hover:bg-gray-50/50 transition-colors">
                                                    ...
                                                    <td className="px-4 py-4 text-gray-500 border-b border-gray-50 group-last:border-none">
                                                        {ride.rider_name || ride.customer || 'N/A'}
                                                    </td>
                                                    <td className="px-4 py-4 text-gray-500 border-b border-gray-50 group-last:border-none">
                                        ...
                                        {isEarningsLoading ? (
                                            <tr>
                                                <td colSpan="6" className="text-center py-10">
                                                    <div className="flex justify-center items-center gap-2">
                                                        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-yellow-500"></div>
                                                        <span className="text-gray-400">Loading earnings...</span>
                                                    </div>
                                                </td>
                                            </tr>
                                            ...
                                            earningsHistory.map((earning, idx) => (
                                                <tr key={idx} className="group hover:bg-gray-50/50 transition-colors">
                                                    ...
                                                    <td className="px-4 py-4 text-gray-500 border-b border-gray-50 group-last:border-none text-xs">
                                                        {earning.payment_type || earning.type || 'Ride Earning'}
                                                    </td>
                                                    <td className="px-4 py-4 border-b border-gray-50 group-last:border-none">
                        ...
                        <div className="flex flex-col gap-6">
                            <div className="flex justify-between items-center bg-gray-50 p-4 rounded-xl border border-gray-100">
                                ...
                                <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider ${
                                    driver.document_status === 'approved' || driver.verified 
                                    ? 'bg-green-100 text-green-700' 
                                    : driver.document_status === 'declined'
                                    ? 'bg-red-100 text-red-700'
                                    : 'bg-yellow-100 text-yellow-700'
                                }`}>
                                    {driver.document_status || (driver.verified ? 'Approved' : 'Pending/Unverified')}
                                </span>
                            </div>
                                ...
                                {[
                                    ...
                                    <div key={idx} className="border border-gray-100 rounded-xl overflow-hidden bg-white shadow-sm flex flex-col">
                                        ...
                                        <div className="p-4 flex-grow flex items-center justify-center bg-gray-50/30 min-h-[160px]">
                                            {driver[doc.key] ? (
                                                <a href={driver[doc.key]} target="_blank" rel="noopener noreferrer" className="block w-full h-full">
                                                    <img 
                                                        src={driver[doc.key]} 
                                                        alt={doc.label} 

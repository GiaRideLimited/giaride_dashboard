...
const DriversContent = () => {
  ...
  const handleStatusUpdate = (reference, currentStatus, firstName) => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to ${action} ${firstName}'s account?`,
      )
    )
      return;
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg flex flex-col justify-between">
            <div className="flex justify-between items-start mb-2 pb-3 border-b border-[#A3A3A333]">
              <h3 className="text-base sm:text-lg font-medium text-gray-700">
                Total {entityTypeForModal.toLowerCase()}s
              </h3>
              <TodayTag />
            </div>
            <div>
              <p className="text-3xl sm:text-[28px] font-bold">
                {driversStats?.data?.total || "--"}
              </p>
            </div>
          </div>
          <div className="bg-white p-6 rounded-xl shadow-lg flex flex-col justify-between">
            <div className="flex justify-between items-start mb-3 pb-3 border-b border-[#A3A3A333]">
              <h3 className="text-base sm:text-lg font-medium text-gray-700">
                Active {entityTypeForModal.toLowerCase()}s
              </h3>
              <TodayTag />
            </div>
            <div>
              <p className="text-3xl sm:text-[28px] font-bold">
                {driversStats?.data?.active || "--"}
              </p>
            </div>
          </div>
        </div>
              ...
              {isLoading ? (
                ...
                (() => {
                  const filteredDrivers = driversList.filter((driver) => {
                    // Filter by Tab
                    if (activeTab === "Pending" && driver.status !== "pending")
                      return false;
                    ...
                    return (
                      ...
                      (driver.reference &&
                        driver.reference.toLowerCase().includes(searchLower)) ||
                      (driver.status &&
                      ...
                      (driver.gender &&
                        driver.gender.toLowerCase().includes(searchLower))
                    );
                  });
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-4 capitalize">
                          <span
                            className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${driver.status === "active" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                          >
                        ...
                        <td className="px-4 py-4 text-center relative">
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              setOpenDropdownId(
                                openDropdownId === driver.id ? null : driver.id,
                              );
                            }}
                          >
                          ...
                          {openDropdownId === driver.id && (
                            ...
                            >
                              ...
                              <button
                                ...
                                onClick={() => {
                                  toast.success(
                                    `Sending notification to ${driver.first_name}`,
                                  );
                                  setOpenDropdownId(null);
                                }}
                              >
